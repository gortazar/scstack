...
#  }
  ...
  file {"$installFolder/archiva-db-setup.sql":
    content => template('scstack/tomcat/archiva-db-setup.sql.erb'),
  }
...
#      File["$installFolder/$jenkins"], 
  ...
  file {"$installFolder/gerrit-db-setup.sql":
    content => template('scstack/tomcat/gerrit-db-setup.sql.erb'),
  }
...
#  }
  ...
  file { "/etc/init/gerrit.conf": 
    content => template("scstack/tomcat/gerrit.conf.erb"),
  }
